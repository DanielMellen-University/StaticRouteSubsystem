...
This program is for Windows 11. Run it from an Administrator terminal because it reconciles persistent routes.
...
Use Python 3.9 or newer and Windows PowerShell with its built-in NetTCPIP module from an Administrator terminal:

```bat
py static_route_subsystem.py
```

The Python program uses only the Python standard library. Windows PowerShell and NetTCPIP are supplied by Windows 11.
...
The route manager starts Windows PowerShell by its full path under the Windows system directory. It does not search the working directory or `PATH` for the executable.
